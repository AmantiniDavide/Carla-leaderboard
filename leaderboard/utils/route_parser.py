...
def parse_bool_attribute(value, default):
    """Parse an optional XML boolean attribute."""
    if value is None:
        return default
...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, routes_subset=''):
        ...
        for route in tree.iter("route"):
            ...
            route_config.background_traffic = parse_bool_attribute(
                route.attrib.get('background_traffic', route.attrib.get('background_activity')), True)
            route_config.spawn_parked_vehicles = parse_bool_attribute(
                route.attrib.get('parked_vehicles'), True)
