...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        self.timeout = 10000
        self._enable_background_traffic = getattr(config, 'background_traffic', True)
        self._enable_parked_vehicles = getattr(config, 'spawn_parked_vehicles', True)
    ...
    def _get_parking_slots(self, max_distance=100, route_step=10):
        ...
        if not self._enable_parked_vehicles:
            self.available_parking_locations = []
            return
    ...
    def _create_behavior(self):
        ...
        if self._enable_background_traffic:
